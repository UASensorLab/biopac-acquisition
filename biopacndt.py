...
class AcqNdtChannel:
	...
	def __setattr__(self, name, value):
		...
		# TODO fix
		'''
		#if (len(self.__dict__.keys()) == 0) or (self.__dict__[name] is None):
...
class AcqNdtServer:
	...
	def __init__(self, host, port):
		...
		self.__RPC =  xmlrpclib.ServerProxy(self.__rpcServerURL)
	...
	def LoadTemplate(self, filename):
		...
		binaryFile = xmlrpclib.Binary(data.encode())
...
class AcqNdtDataServer(SocketServer.ThreadingMixIn, SocketServer.TCPServer):
	...
	def RegisterCallback(self, name, callback):
		...
		if name in self.__callBacks.keys():
	...
	def RegisterCloseCallback(self, name, callback):
		...
		#if self.__closedCallBacks.has_key(name):
		if name in self.__closedCallBacks.keys():
				...
				if 0 != recvLen:
					...
					if len(buf) != recvLen:
						...
						if len(buf) == 0:
							...
							for (name, func) in closeCallbacks.iteritems():
								func()
...
def FindAcqNdtServers():
	...
	finderSock.sendto(str.encode(sendMsg), hostPort)
	...
	try:
		while True:
			...
			if recvMsg.startswith(str.encode(messagePrefix)):
				tokens = recvMsg.split(str.encode(":"))
				if len(tokens) == 2:
...
def main():
	...
	print ("MP Sampling Rate: %s" % acqServer.getSamplingRate())
